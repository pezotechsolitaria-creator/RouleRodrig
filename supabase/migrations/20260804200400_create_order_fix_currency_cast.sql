-- Fix: stores.currency is character(3) (bpchar), but the function's RETURNS
-- TABLE declares `currency text` — Postgres won't implicitly match bpchar to
-- text across a RETURN QUERY boundary. Found immediately via live SQL
-- testing before any application code was built on top of this RPC.
