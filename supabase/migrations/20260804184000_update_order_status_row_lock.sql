-- Adversarial review finding (M4): update_order_status() read orders.status
-- without FOR UPDATE, then unconditionally overwrote it later in the same
-- function. Two concurrent calls on the same order (e.g. merchant clicks
-- "cancel" while another tab/request marks it "preparing") could both read
-- the same starting status, both pass their own legality check against that
-- stale read, and whichever UPDATE commits last wins outright — silently
-- discarding the other transition with no error. FOR UPDATE serializes
-- concurrent calls on the same order row: the second call blocks until the
-- first commits, then re-reads the now-current status, so its legality
-- check is never based on stale data.
