-- M6 — create_order v4.
--
-- Architecture update: MCB Juice removed as a merchant payment method.
-- Marketplace payments are Cash and Direct Bank Transfer only. Vehicle rentals
-- and place bookings keep PayPal/cards and are untouched by this file.
--
-- Idempotent: the DROP targets the superseded 7-argument signature (already
-- gone in any environment that has run v2+), and the function itself is a
-- CREATE OR REPLACE, so replaying this migration is a no-op.
...
begin
  ...
  -- pickup / customer_delivery / rr_delivery — only the last one is charged for.
  ...
  -- No card rails and no mcb_juice. One of three independent gates, alongside
  -- the Zod schema and the CHECK constraint on payments.
  ...
  if not coalesce((v_store.fulfillment ->> (case when p_fulfillment='pickup' then 'pickup' else 'delivery' end))::boolean,false) then
  ...
  -- GPS is the delivery address for BOTH delivery modes: the RR team needs it to
  -- navigate, and a customer's own driver needs something shareable.
  if p_fulfillment in ('customer_delivery','rr_delivery') then
    ...
    -- The fee is read from platform config here and never from the client.
  ...
  -- Release genuinely abandoned checkouts for these variants. Honours
  -- auto_release_at, which submit_payment_receipt() nulls the moment a customer
  -- reports payment, so an order backed by real money is never swept.
  for v_stale in
    select distinct o.id from orders o join order_items oi on oi.order_id=o.id
    where o.status='pending_payment' and o.auto_release_at is not null
  ...
  values (v_order_number, p_store_id, v_customer, nullif(trim(p_customer_name),''), nullif(trim(p_customer_phone),''),
    ...
    nullif(trim(p_delivery_instructions),''), now()+interval '48 hours')
  returning id into v_order_id;

  -- Variants are locked in a stable id order so concurrent carts sharing items
  -- cannot deadlock, and stock is validated under that lock.
  ...
  if not v_store.tax_inclusive then v_tax := round(v_subtotal*coalesce(v_store.default_tax_rate,0))::integer; end if;
  v_total := v_subtotal + v_tax + v_delivery_fee;
  -- DEPRECATED: revenue is a monthly merchant subscription, not a commission.
  -- Written as 0 to keep the NOT NULL column valid; dropping it is future work.
