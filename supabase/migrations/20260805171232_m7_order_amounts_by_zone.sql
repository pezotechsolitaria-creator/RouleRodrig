-- order_amounts() remains the single source of truth for pricing, now
-- region-aware. The zone ID is the only new input; the FEE is still read from
-- the database, so a tampered client cannot influence what delivery costs.
...
begin
  ...
  -- Only the Roulé Rodrigues delivery network charges. Pickup and a
  ...
  if p_fulfillment = 'rr_delivery' then
    ...
    if coalesce(v_settings.delivery_enabled, false) then
      if p_zone_id is null then
        raise exception using errcode = 'RR005', message = 'Choose the area we are delivering to.';
      end if;
      select * into v_zone from delivery_zones where id = p_zone_id and is_active;
      if v_zone.id is null then
        raise exception using errcode = 'RR005', message = 'We do not deliver to that area right now.';
      end if;
      v_fee := v_zone.fee;
    end if;
  end if;
...
begin
  ...
  select * into v_amounts from order_amounts(p_store_id, v_subtotal, p_fulfillment, p_zone_id);
  return query select v_subtotal, v_amounts.tax, v_amounts.delivery_fee, v_amounts.total, v_store.currency::text;
end;
$$;

revoke execute on function order_amounts(uuid, integer, text, uuid) from public, anon;
revoke execute on function quote_order(uuid, jsonb, text, uuid) from public, anon;
grant execute on function quote_order(uuid, jsonb, text, uuid) to authenticated;

-- Drop the superseded 3-arg signatures so no caller can silently bind to the
-- old, zone-unaware pricing.
drop function if exists order_amounts(uuid, integer, text);
drop function if exists quote_order(uuid, jsonb, text);
