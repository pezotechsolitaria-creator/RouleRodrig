-- M6 phase 2 — create_order v3.
--
-- Business-rule changes vs v2:
--   * THREE delivery choices (pickup / customer_delivery / rr_delivery) instead
--     of two. Only rr_delivery incurs the platform delivery fee; a customer who
--     arranges their own driver pays nothing for delivery.
--   * Both delivery modes require and persist GPS — the RR team needs it to
--     navigate, and a customer's own driver needs it to be shareable.
--   * MCB Juice QR is gated by its own accepts_juice_qr toggle rather than
--     riding on the bank-transfer flag.
-- The shop capability flag still only distinguishes pickup vs delivery, so both
-- delivery modes map onto stores.fulfillment->>'delivery'.
...
begin
  ...
  if not coalesce((v_store.fulfillment ->> (case when p_fulfillment = 'pickup' then 'pickup' else 'delivery' end))::boolean, false) then
    raise exception using errcode = 'RR005', message = format('This shop does not offer %s.', replace(p_fulfillment, '_', ' '));
  end if;
  ...
  if p_provider = 'mcb_juice' and not coalesce(v_pay.accepts_juice_qr, false) then
    raise exception using errcode = 'RR009', message = 'This shop does not accept MCB Juice.';
  end if;
  ...
  if p_fulfillment in ('customer_delivery', 'rr_delivery') then
    ...
    if p_fulfillment = 'rr_delivery' then
      ...
      if not coalesce(v_settings.delivery_enabled, false) then
        raise exception using errcode = 'RR005', message = 'Roule Rodrigues delivery is not available at the moment.';
      end if;
