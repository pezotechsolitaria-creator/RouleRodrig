-- ============================================================================
-- 0007 — create_product() RPC (Milestone 3: product management)
-- ----------------------------------------------------------------------------
-- Same atomicity rationale as onboard_merchant(): a product + its default
-- variant + optional opening stock movement are created in one transaction,
-- so a failure between inserts can never leave a product with no variant
-- (which would be unsellable and unlistable — has_variants=false products
-- are expected to always have exactly one variant).
--
-- Unlike onboard_merchant(), store_id here IS a caller-supplied parameter
-- (the API route resolves it server-side from the caller's own staff
-- membership before calling this, but this function is SECURITY DEFINER and
-- therefore bypasses RLS internally — it must not assume the caller already
-- verified ownership. is_store_staff(p_store_id) re-checks it independently,
-- so this function is safe to call with any store_id and simply rejects
-- ones the caller doesn't staff.
-- ============================================================================
