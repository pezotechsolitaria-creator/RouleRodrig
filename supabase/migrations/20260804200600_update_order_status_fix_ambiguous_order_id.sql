-- Same ambiguous-column class as the payment RPC fix in this batch:
-- RETURNS TABLE(order_id uuid, status text) shadows the bare `order_id`
-- column reference in the new (M5) stock-release INSERT. Found via live
-- testing before this branch had ever actually executed.
...
begin
  ...
  if p_new_status = 'cancelled' and v_current_status not in ('cancelled', 'refunded') then
    ...
    select oi.variant_id, oi.quantity, 'restock', p_order_id, 'released: order cancelled by merchant'
