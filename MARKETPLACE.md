...
> | `app/api/marketplace/qr/redeem/route.ts`, an HMAC token, an in-app scanner | Different shape. The handoff lives at `app/api/merchant/pickup/{preview,redeem}` (M28/M30). The token is an 8-character code, stored hashed, shown to the customer as BOTH a QR and characters; the QR encodes a URL so the merchant's own camera app scans it and lands on `/merchant/pickup`, where they preview the order before confirming |
