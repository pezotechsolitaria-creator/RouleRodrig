...
function LoginForm() {
  ...
  async function resendConfirmation() {
    ...
    try {
      res = await fetch("/api/auth/email-link", {
        ...
        body: JSON.stringify({ email: email.trim(), kind: "confirm", next: next }),
      });
    } catch {
  ...
  async function sendReset(e: React.FormEvent) {
    ...
    setBusy("email");
    setError(null);
    // Sent by US through Brevo rather than by Supabase Auth over its own SMTP.
    // See app/api/auth/email-link/route.ts for why that matters for a password
    // reset specifically.
    try {
      await fetch("/api/auth/email-link", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: email.trim(), kind: "recovery", next }),
      });
    } catch (err) {
