...
export default async function CommandCenterPage() {
      ...
      <main className="min-h-screen bg-dark px-4 pb-16 pt-10 text-offwhite">
        <div className="mx-auto max-w-3xl rounded-2xl border border-orange-400/30 bg-orange-400/5 p-6">
          <p className="font-syne text-lg font-bold">
            The Command Center needs the service-role key
          </p>
          <p className="mt-2 font-dm text-sm text-muted">
            SUPABASE_SERVICE_ROLE_KEY is unset on this environment, so live
            counts cannot be read. The content studio still works:{" "}
            <Link href="/admin/content" className="text-yellow underline">
              open it here
            </Link>
            .
          </p>
  ...
  const [
    ...
    admin
      .from("bookings")
      .select("id, name, scooter, status, created_at")
      .order("created_at", { ascending: false })
      .limit(5),
    admin
      .from("place_bookings")
      .select("id, name, place_name, status, created_at")
      .order("created_at", { ascending: false })
      .limit(5),
    admin
      .from("lead_events")
      .select("kind, target_name, created_at")
      .order("created_at", { ascending: false })
      .limit(6),
    admin
      .from("contact_submissions")
  ...
  const feed = mergeActivity(
    [
      (
        (recentOrders.data ?? []) as Record<string, string | number>[]
      ).map<ActivityEvent>((o) => ({
      ...
      (
        (recentBookings.data ?? []) as Record<string, string>[]
      ).map<ActivityEvent>((b) => ({
        at: b.created_at,
      ...
      (
        (recentPlaces.data ?? []) as Record<string, string>[]
      ).map<ActivityEvent>((b) => ({
        at: b.created_at,
        line: `Experience booking — ${b.name ?? "someone"} · ${b.place_name ?? ""} · ${b.status}`,
        href: "/admin/content#place_bookings",
      })),
      ((recentLeads.data ?? []) as Record<string, string>[]).map<ActivityEvent>(
        (l) => ({
          at: l.created_at,
      ...
      (
        (recentContacts.data ?? []) as Record<string, string>[]
      ).map<ActivityEvent>((c) => ({
        at: c.created_at,
  ...
  const kpis = [
    {
      icon: Banknote,
      label: "Revenue today",
      value: `Rs ${centsToDecimalString(revenueToday)}`,
    },
    {
      icon: ClipboardList,
      label: "Orders today",
    ...
    },
    {
      ...
      value: String(count(todaysBookings)),
    },
    {
      icon: Inbox,
      label: "Open order queue",
      value: String(count(openOrders)),
    },
  ];
            ...
            return (
              ...
              >
                <p className="flex items-center gap-1.5 font-bebas text-[10px] tracking-[0.22em] text-muted">
                  <Icon size={12} className="text-yellow" />{" "}
                  {k.label.toUpperCase()}
                </p>
                <p className="mt-1 font-syne text-xl font-extrabold text-offwhite">
                  {k.value}
                </p>
              </div>
            );
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_1fr]">
          ...
          <section>
            <h2 className="flex items-center gap-2 font-syne text-base font-extrabold">
              <AlertTriangle size={16} className="text-yellow" /> Requires
              attention
            </h2>
            {attention.length === 0 ? (
              <div className="mt-3 rounded-2xl border border-white/10 bg-dark-card px-5 py-8 text-center">
                ...
                <p className="mt-1 font-dm text-xs text-muted">
                  Nothing is waiting on you. New items appear here the moment
          ...
          {/* ── Which pages are working ────────────────────────────────
              Above the activity feed, below the attention queue: the feed says
              what happened in the last hour, this says what has been happening
              for a month, and the queue says what needs a person right now.
              A client component because it fetches on demand and lets the
              operator change the window without reloading the whole desk. */}
          <PageAnalytics />
          ...
          <section>
            ...
            {feed.length === 0 ? (
              <p className="mt-3 rounded-2xl border border-white/10 bg-dark-card px-5 py-8 text-center font-dm text-sm text-muted">
                Quiet so far. Orders, bookings and leads appear here as they
                happen.
              </p>
            ) : (
              <ol className="mt-3 space-y-1">
                {feed.map((e, i) => (
                  <li key={`${e.at}-${i}`}>
                    ...
                    >
                      <span className="min-w-0 truncate font-dm text-[13px] text-offwhite/85">
                        {e.line}
                      </span>
