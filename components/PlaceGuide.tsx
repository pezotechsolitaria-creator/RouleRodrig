      ...
      <header className="border-b border-dark-border bg-gradient-to-b from-yellow/[0.06] to-transparent px-5 py-16 md:py-24">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            >
              Rent a scooter to get there <ArrowRight size={16} />
            </Link>
            <Link
          ...
          {places.map((p, i) => {
            ...
            return (
              <article key={p.id} className="scroll-mt-24">
                ...
                {p.description && (
                  <p className="mt-4 font-dm text-muted leading-relaxed">{p.description.trim()}</p>
                )}
                {p.story && (
                  <p className="mt-3 font-dm text-muted leading-relaxed">{p.story.trim()}</p>
                )}
                    ...
                    {photos.map((src, n) => (
                      ...
                      >
                        <Image
                          ...
                          alt={`${p.name.trim()} in Rodrigues Island — photo ${n + 1} of ${photos.length}`}
                          fill
